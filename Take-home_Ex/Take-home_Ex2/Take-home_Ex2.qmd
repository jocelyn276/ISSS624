...
According to summary result, we can find that there are 13 rows have NAs. Because the numbers of their total water points are 0, and respective percentage shows as NaN. The code chunk below is used to fill NaN with 0.
...
### 4.5.1 Selecting the optimal clustering algorithm by hierarchical clustering
...
### 4.5.2 Determining Optimal Clusters
...
### 4.5.3 Interpreting the dendrogram
...
### 4.5.5 Mapping the clusters formed
...
hcluster_plot <- tm_shape(lga_cluster) +
  tm_polygons("Hierarchical Cluster", title = "Hierarchical Clusters") +
  tm_layout(main.title = "Conventional Hierarchical Clustering",
...
The choropleth map shows the clustering result geographically. The areas are very fragmented, which is a limitation of non-spatial clustering algorithm.

## 4.6 Selecting the optimal clustering algorithm by K-means clustering
...
### 4.6.2 **Average Silhouette Method**
...
### 4.6.4 Mapping the clusters formed
...
### 5.3.3 Visualising the clusters in choropleth map
...
skatercluster_plot <- tm_shape(nigeria_wp_sf_spatialcluster) +
  tm_polygons("SP_CLUSTER", title = "SKATER Clusters") +
...
### 6.1.2 Mapping the clusters formed
...
wardlikehcluster_plot <- tm_shape(nigeria_wp_sf_ngeo_cluster) +
  tm_polygons("Ward-Like HCLUSTER", title = "Ward-Like Hierarchical Clusters") +
...
## 6.2 Spatially Constrained Clustering
...
Geocluster_plot <- tm_shape(nigeria_wp_Gcluster) +
  tm_polygons("Geo CLUSTER", title = "Geo Hierarchical Clusters") +
...
```{r}
#| warning: false
tmap_arrange(hcluster_plot, skatercluster_plot,
